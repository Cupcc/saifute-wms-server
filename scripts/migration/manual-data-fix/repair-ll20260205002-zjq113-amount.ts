...
const DOCUMENT_NO = "LL20260205002";
const LINE_ID = 239;
const MATERIAL_CODE = "zjq113";
const EXPECTED_QUANTITY = "5.000000";
const EXPECTED_UNIT_COST = "8.85";
const EXPECTED_AMOUNT = "44.25";
const WRONG_LINE_UNIT_PRICE = "8796.90";
const WRONG_LINE_AMOUNT = "43984.50";
const UPDATED_BY = "manual-repair-ll20260205002-zjq113-amount-20260517";
const DRY_RUN_REPORT_FILE_NAME =
  "repair-ll20260205002-zjq113-amount-dry-run-report.json";
const EXECUTE_REPORT_FILE_NAME =
  "repair-ll20260205002-zjq113-amount-execute-report.json";
...
function validateContext(
  ...
  if (
    ...
    !decimalEq(context.lineUnitPrice, EXPECTED_UNIT_COST)
  ) {
    blockers.push(
      `目标明细单价既不是待修复值 ${WRONG_LINE_UNIT_PRICE}，也不是确认值 ${EXPECTED_UNIT_COST}: ${context.lineUnitPrice}`,
    );
  }

  if (
    !decimalEq(context.lineAmount, WRONG_LINE_AMOUNT) &&
    !decimalEq(context.lineAmount, EXPECTED_AMOUNT)
  ) {
    blockers.push(
      `目标明细金额既不是待修复值 ${WRONG_LINE_AMOUNT}，也不是确认值 ${EXPECTED_AMOUNT}: ${context.lineAmount}`,
    );
  }

  if (!decimalEq(context.outLogUnitCost, EXPECTED_UNIT_COST)) {
    blockers.push(
      `库存出库流水单价不是 ${EXPECTED_UNIT_COST}: ${context.outLogUnitCost ?? "null"}`,
    );
  }

  if (!decimalEq(context.outLogCostAmount, EXPECTED_AMOUNT)) {
    blockers.push(
      `库存出库流水金额不是 ${EXPECTED_AMOUNT}: ${context.outLogCostAmount ?? "null"}`,
    );
  }
  ...
  const mismatchedSourceUsages = activeSourceUsages.filter(
    (usage) => !decimalEq(usage.sourceUnitCost, EXPECTED_UNIT_COST),
  );
  if (mismatchedSourceUsages.length > 0) {
    blockers.push(
      `存在非 ${EXPECTED_UNIT_COST} 的有效来源占用: usageIds=${mismatchedSourceUsages
    ...
    await withPoolConnection(pool, async (connection) => {
      ...
      const report = {
        ...
        confirmedBusinessFact:
          "仓库管理员确认：实际领料为 5 个 × 8.85 = 44.25；旧库小计和 instruction 说明均为错误数据。",
        updatedBy: UPDATED_BY,
        expected: {
          quantity: EXPECTED_QUANTITY,
          unitCost: EXPECTED_UNIT_COST,
          amount: EXPECTED_AMOUNT,
        },
        knownWrongValues: {
          lineUnitPrice: WRONG_LINE_UNIT_PRICE,
          lineAmount: WRONG_LINE_AMOUNT,
        },
        blockers,
        before,
        beforeTotals,
