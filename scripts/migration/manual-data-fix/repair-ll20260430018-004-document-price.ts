...
const DOCUMENT_NO = "LL20260430018";
const LINE_ID = 1764;
const MATERIAL_CODE = "004";
const MATERIAL_NAME = "不锈钢丝网";
const EXPECTED_QUANTITY = "12.000000";
const CONFIRMED_UNIT_COST = "0.60";
const CONFIRMED_AMOUNT = "7.20";
const WRONG_LINE_UNIT_PRICE = "1.05";
const WRONG_LINE_AMOUNT = "12.60";
const UPDATED_BY = "manual-repair-ll20260430018-004-document-price-20260519";
const DRY_RUN_REPORT_FILE_NAME =
  "repair-ll20260430018-004-document-price-dry-run-report.json";
const EXECUTE_REPORT_FILE_NAME =
  "repair-ll20260430018-004-document-price-execute-report.json";
...
function validateContext(
  ...
  if (context.materialCode !== MATERIAL_CODE) {
    blockers.push(`目标物料编码不是 ${MATERIAL_CODE}: ${context.materialCode}`);
  }
    ...
    await withPoolConnection(pool, async (connection) => {
      ...
      const report = {
        ...
        confirmedBusinessFact:
          "业务确认：LL20260430018 的 004 不锈钢丝网单据价 1.05 为误填，应改为库存来源价格 0.60。",
      ...
      console.log(
        `LL20260430018 004 document price repair ${execute ? "execute" : "dry-run"} completed. blockers=${blockers.length}, report=${reportPath}`,
      );
    });
  } finally {
