...
如果目标是让 AI 自主推进并自证完成，requirement 还需要承担“可执行 contract”职责：至少把本次范围、非目标、编号化验收标准 `[AC-*]`、证据类型期待和完成定义说清楚。详细测试步骤仍不写在 requirement 中。
...
| `req-*.md` | 单次任务、切片或会话级需求，以及简洁 `当前进展`、编号化验收标准与 requirement 级 `验收状态` | 长期主题边界、执行日志、详细测试步骤 |
...
- 如果用户要求“AI 自主完成并提供完整测试报告”，requirement 仍保持简洁，但必须至少说清楚：范围、非目标、`[AC-*]`、证据类型、完成定义。
...
| 切片级 | 本次交付范围、非目标、`[AC-*]`、证据类型、当前进展、聚合验收状态、待确认 | `_template.md` |
...
- `req-*.md` 的 `[AC-*]` 应是用户可读、可判定、可映射到 acceptance case 的业务标准，而不是实现步骤。

## 编写约定

- 全文用中文撰写；路径、文件名、 metadata 可保留英文。
- 内容保持简洁，优先把用户需求讲清楚，不擅自展开实现方案。
- 在需要 AI 自主交付时，优先把 requirement 写成“判定合同”而不是“散文 PRD”。
- `当前进展` 只写关键阶段状态，不写成长执行日志。
- `req-*.md` 建议至少包含 `用户需求`、`交付范围`、`验收标准`、`当前进展`、`待确认`。
...
- 若 `Delivery mode = autonomous`，建议在 requirement 中补齐：
  - `交付范围`：明确 in-scope 与 out-of-scope
  - `验收标准`：逐条编号为 `[AC-*]`
  - `验收证据要求`：只写证据类型，不写步骤
  - `完成定义`：明确什么叫完成、条件通过、阻塞

## 验收文档关系

- `none` 模式：requirement 只写 `验收状态: 已跳过` 或保持未进入验收，不创建独立验收资产。
- `light` 模式：详细验收证据通常留在 task doc 的 `## Acceptance`，requirement 只保留聚合结果。
- `full` 模式：详细验收证据主要留在 `docs/acceptance-tests/specs/**` 与 `docs/acceptance-tests/runs/**`，其中 `run` 是本次交付的完整测试报告；requirement 只保留聚合结果。
- requirement 的 `验收状态` 必须反映 linked tasks 的聚合结论，不能用某一个 task 的最后一次写入覆盖事实。

## 自主交付模式

当用户明确表达以下意图之一时，按“自主交付模式”写 requirement，并默认把验证闸门抬到 `full`：
...
自主交付模式下的 requirement 最少要满足：

1. 已 `confirmed`，不存在未决 scope 漂移。
2. 写清 `In scope` 与 `Out of scope / non-goals`。
3. 所有验收标准都编号为 `[AC-*]`。
4. 每个 `[AC-*]` 至少声明一种证据类型。
5. 写清 `完成定义`，避免“代码写完 = 完成”。

如果这些信息缺失，不应直接进入 coder 实现阶段。
