...
- 理由摘要：
  ...
  - `2026-04-15` follow-on 重构已落地：`RD handoff` 直接并入 `研发项目` 领域和项目汇总，不再保留 `rdHandoffItems` 历史桶；live API 与自动化证据冻结在 automated acceptance evidence。
...
| 2026-04-15 10:48 CST | `-` | `.env.dev`; backend `http://127.0.0.1:8112`; live API + focused automated evidence (`typecheck + reporting tests + build + e2e`) | `passed` |
