...
| 业务语义         | 数据类型            | 说明                              |
| ------------ | --------------- | ------------------------------- |
| 数量           | `DECIMAL(18,6)` | 所有数量字段统一 6 位小数,适用于"个/千克/米"等单位 |
| 金额 / 单价 / 成本 | `DECIMAL(18,4)` | 所有金额字段统一 4 位小数                  |

前端显示约定：数量一律经 `web/src/utils/format.js` 的 `formatQty` 渲染（整数不带小数、小数去尾零），禁止在视图内自建 `toFixed` 数量格式化。
