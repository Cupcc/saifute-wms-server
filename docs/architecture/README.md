...
| `20-wms-database-tables-and-schema.md`          | Frozen database-table and schema baseline for shared core plus transaction families, together with the related domain-flow and state semantics. Read this before implementing any document-flow, inventory, or approval surface.                                                                                                                          |
...
6. `docs/architecture/40-code-quality-governance.md` — code quality rules, thresholds, quality gates, and review criteria; read before writing or reviewing any code
7. Per-module file under `docs/architecture/modules/<module>.md` — module-specific boundaries, responsibilities, current implementation maturity, and cross-module constraints
...
- Use `docs/tasks/`** to capture execution history, review notes, and temporary rollout detail. When a task closes, keep only the lasting architectural truth in `docs/architecture/**`.
