...
负责领料单、退料单、报废单的统一建模与实现。该模块把车间物料流转能力收拢为单一领域。
...
本节冻结领料单新增、修改、作废的业务审计口径；详细执行步骤见
`docs/tasks/task-20260922-workshop-material-audit-history.md`。

- `sys_oper_log` 记录操作事实：真实登录用户 ID / 账号、时间、IP、请求编号、接口动作、成功或失败及错误摘要。业务字段中的领料人、经办人不替代操作账号。
- `document_change_log` 记录不可变业务版本：单据类型、单据号、from/to revision、修改原因、修改前后完整快照和规范化逐字段差异。新增以 `null → v1` 记录，修改以 `vN → vN+1` 记录，作废以当前版本到作废状态记录。
- `workshop_material_order_line` 必须有稳定的 `lineKey`。改单即使删除旧行并重建数据库行，也要在 `document_change_line_map` 中保存旧行到新行的映射；同一物料多行不得只按物料编码猜测对应关系。
- 变更历史通过 `document_change_inventory_link` 关联本次改单产生的旧出库逆流水、冲回流水、新出库流水及来源使用记录。库存流水本身仍只由 `inventory-core` 写入。
- 领料改单的读取、版本检查、旧库存逆操作、来源释放、新明细写入、新库存出库和历史写入必须处于同一个数据库事务；任一步失败整体回滚。操作日志继续允许异步失败，但业务历史不得异步丢失。
- 更新和作废必须提交 `expectedRevisionNo` 并在事务内做乐观锁检查，冲突返回 HTTP 409。数量、物料、单价、日期、车间、经办人或备注变化时必须填写修改原因。
- 变更历史只读，不提供普通修改、删除、清空接口。历史数据补录必须显式标注“依据库存流水重建”，不得伪造原始修改原因。
